...
func init() {
	...
	pool.SubmitTasks("sendMessageToClient", conf.Conf.ClientPoolSize, func(ctx context.Context) (err error) {
		Hub.sendMessageToClient()
		return nil
	})
	pool.SubmitTasks("masterUnregisterHandler", conf.Conf.MasterPoolSize>>6+2, func(ctx context.Context) (err error) {
		Hub.masterUnregisterHandler()
		return nil
	})
	pool.SubmitTasks("masterHandler", conf.Conf.MasterPoolSize>>6+2, func(ctx context.Context) (err error) {
		Hub.masterHandler()
		return nil
	})
	pool.SubmitTasks("clientUnregisterHandler", conf.Conf.ClientPoolSize>>6+2, func(ctx context.Context) (err error) {
		Hub.clientUnregisterHandler()
		return nil
	})
	pool.SubmitTasks("clientHandler", conf.Conf.ClientPoolSize, func(ctx context.Context) (err error) {
		Hub.clientHandler()
		return nil
	})
	pool.SubmitTasks("sendMessageToMaster", conf.Conf.MasterPoolSize, func(ctx context.Context) (err error) {
		Hub.sendMessageToMaster()
		return nil
	})
	pool.SubmitTasks("handleMasterMessage", conf.Conf.MasterPoolSize, func(ctx context.Context) (err error) {
		Hub.handleMasterMessage()
		return nil
	})
...
func (h *webSocketHub) listenMaterMessage(conn *websocket.Conn) {
	conn.OnMessage(func(c *websocket.Conn, messageType websocket.MessageType, data []byte) {
		...
		case websocket.TextMessage:
			...
			h.masterInMessageChan <- &Message{Conn: conn, Data: data}
		default:
